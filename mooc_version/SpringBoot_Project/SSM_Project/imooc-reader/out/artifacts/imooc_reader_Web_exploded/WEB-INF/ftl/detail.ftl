            ...
            <#if loginMember ??>
                $("*[data-read-state]").click(function (){
                    ...
                    $.post("/update_read_state", {
                        ...
                        bookId:${book.bookId},
                        readState:readState
                    },function (json) { //从服务器中获取到json数据
                        if (json.code == "0"){
                            $("*[data-read-state]").removeClass("highlight");//移除所有阅读状态的高亮状态
                            $("*[data-read-state='" + readState + "']").addClass("highlight");//将服务器传来的json数据中的readState使用字符串拼接的方法赋值给自定义属性data-read-state,用于标记高亮
                        }
                    }, "json")
                });

                //点击写短评，跳出写短评的对话框，该对话框可以进行星型评分和内容评价
